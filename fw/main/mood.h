...
extern int16_t g_sig_hist[SIG_HIST_N];   // RSRP
extern int16_t g_sinr_hist[SIG_HIST_N];  // SINR（与 RSRP 同步推，共用 head/cnt）
...
// IMU 检测到运动时调用：刷新"最近有动静"时间戳，久闲计时清零（拿起来不打盹）
void mood_note_motion(void);
